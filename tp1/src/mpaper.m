...
if nargin >= 1 & ischar(varargin{1}),
    ...
    if nargin >=1, options = varargin{1}; else options=[]; end
...
title('DRAW:Left          SAVE:Right','FontSize',14,'Color','blue');
...
if( x(1) > 0 & x(1) < 1 & x(3) > 0 & x(3) < 1 );
    ...
    if( isempty( l ) ),
        figure(1)
        l = plot( x(1), x(3), '.-' ,'MarkerSize',30 );
        set( l, 'erasemode', 'none', 'tag', 'brush_stoke', 'color', [0.8 0 0]);
